...
describe("Scubbers", () => {
  test("scrub", () => {
    ...
    verify(
      text,
      new Options().withScrubber((t) => StringUtils.replaceAll(t, "2", "two")),
    );
  });
  test("guids", () => {
    const text = {
      people: [
        { id: "58f471f1-8b1f-413c-8971-21cb23bfc8f2", name: "Eve" },
        {
  ...
  test("regex", () => {
    ...
    verifyAsJson(
      text,
      new Options().withScrubber(
        Scrubbers.createReqexScrubber(/\([\s\S]+\)/gi, "crazy"),
      ),
    );
  });
  test("dates", () => {
