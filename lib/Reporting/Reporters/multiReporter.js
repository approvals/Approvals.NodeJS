...
MultiReporter.prototype.canReportOn = function (fileName) {
  return this.reporters.every(function (reporter) {
    return reporter.canReportOn(fileName)
  });
}

MultiReporter.prototype.report = function (approvedFilePath, receivedFilePath) {
  var errors = [];
  this.reporters.forEach(function (reporter) {
    try {
      reporter.report(approvedFilePath, receivedFilePath);
    } catch (err) {
      errors.push(err);
    }
  });
