...
class GenericDiffReporterBase {
  ...
  report(approved, received, options) {
    ...
    if (!options.blockUntilReporterExits) {
      if (reportingLaunchingCircuitBreaker.isLimitExceeded()) {
        return (new NodeDiffReporter()).report(approved, received, options);
      } else {
        reportingLaunchingCircuitBreaker.notifyLaunched();
      }
    }
