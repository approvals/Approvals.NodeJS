...
exports.verify = function (namer, writer, reporterFactory, options) {
  ...
  console.log('failOnLineEndingDifferences', failOnLineEndingDifferences);
  ...
  for (var i = 0, bufferLength = approvedFileBuffer.length; i < bufferLength; i++) {
    if (approvedFileBuffer[i] !== receivedFileBuffer[i]) {
      ...
      approvedFileBufferNormalized = approvedFileBuffer.toString().replace(/\r\n/g, '\n');
      receivedFileBufferNormalized = receivedFileBuffer.toString().replace(/\r\n/, '\n');
