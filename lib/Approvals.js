...
exports.mocha = function() {
  ...
  if (arguments[0]) {
    console.warn("[approvals] The first parameter which usually was given the value '__dirname' has been removed and is now ignored. Please update the call sight by removing the parameter.");
  }

  var Namer = require("./Providers/Mocha/MochaNamer.js");
  var baseVerifier = require("./Providers/BeforeEachVerifierBase.js");

  baseVerifier(Namer, "require('Approvals').mocha();", path.dirname(callsite()[1].getFileName()));
...
exports.jasmine = function() {
  ...
  baseVerifier(Namer, "require('Approvals').jasmine();", path.dirname(callsite()[1].getFileName()));
